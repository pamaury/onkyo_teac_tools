...
#define EM_BLACKFIN 106 /* Blackfin */
...
{
    struct elf_symbol_t *sym = malloc(sizeof(struct elf_symbol_t));
...
{
    struct elf_segment_t *seg = malloc(sizeof(struct elf_section_t));
    ...
    uint32_t fill_addr, uint32_t size, uint32_t pattern, const char *name)
{
    if(pattern != 0x00)
    {
        ...
        uint8_t *p = (void *)&pattern;
        for(size_t i = 0; i < size; i++)
            data[i] = p[i % 4];
        elf_add_load_section(params, fill_addr, size, data, name);
        free(data);
        /* we need to convert it to a load section */
        return;
    }
...
void elf_simplify(struct elf_params_t *params)
{
    ...
    {
        /*
        printf("prev=%d, new=%d, end addr=%#x, new addr=%#x\n", cur_sec->type,
            sections[i].type, cur_sec->addr + cur_sec->size, sections[i].addr);
        */
        /* different type => no */
